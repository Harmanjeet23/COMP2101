...
myhostname=$(hostname) #hostname 
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #finding the name of interface and storing it in variable interface_name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #finding the lan address and storing it in lan_address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') #finding hostname of lan using getent hosts and lan_address
external_ip=$(curl -s icanhazip.com) #getting external ip address from curl.
external_name=$(getent hosts $external_ip| awk '{print $2}') #getting external ip adresses using hosts file by $external_ip.
router_address=$(ip r s default| cut -d ' ' -f 3) #getting default router address.
router_hostname=$(getent hosts $router_address|awk '{print $2}') #Using getent host and $router_address, find hostname of router.
