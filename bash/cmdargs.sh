...
myargs=()
#to identifu the start od the input create variables.
vmodecheck=0
dmodecheck=0
...
while [ $# -gt 0 ]; do
  ...
  -d )
    ...
    [1-5] )
		...
		dmodecheck=$2
		#shifing the command line argument to process the next elememy.
		shift
		;;
    *)
...
#to check the state of the input, use if conditions.
if [ $vmodecheck = 1 ]; then
...
# Print out the myargs array with a label
#to check the state of the input, use if conditions.
if [ $dmodecheck -gt 0 ]; then
  echo "Debug Mode is On with debug level $dmodecheck."
else
